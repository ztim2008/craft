...
function hostingLines(kind: HostingKind, origin: string, port: number): string[] {
  ...
  if (kind === "beget") {
    return [
      ...
      "- Отвяжите домен " + host + " от Крафтума и направьте его на это приложение. Домен ваш, новый не покупаем.",
...
export function deployInstructionTxt(input: InstructionInput): string {
  ...
  lines.push(
    ...
    "5. ДОМЕН — ОН УЖЕ ВАШ",
    "",
    "Домен не покупаем и не забираем. Он остаётся у вас.",
    "На Крафтуме он только привязан к их серверам. Нужно отвязать и направить на ваш хостинг.",
    "",
    "Сначала сайт должен открываться на вашем хостинге (по IP или временному адресу).",
    "Только потом отвязывайте домен от Крафтума — иначе будет пауза, пока DNS не дойдёт.",
    "",
    "Как отвязать:",
    "- В панели Крафтума снимите привязку домена (их NS или A-запись).",
    "- У регистратора, где куплен домен, поставьте A-запись " + domain + " на IP вашего хостинга.",
    "- Если стоят NS Крафтума — верните NS регистратора, затем A-запись.",
    "",
    "A-запись домена " + domain + " должна указывать на IP хостинга.",
    "Пока DNS не обновится (иногда до суток), с телефона сайт может ещё открываться на Крафтуме.",
    "",
